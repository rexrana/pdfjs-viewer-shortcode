...
class Sidebar_Manager {
	...
	public function execute_action( $action, $input ) {
		...
		if ( empty( $name ) ) {
			return new \WP_Error( 'name-missing', __( 'Please enter a name for the sidebar.', 'pdfjs-viewer-shortcode' ) );
		}

		$result = new \WP_Error( 'unknown-action', __( 'Unknown action attempted.', 'pdfjs-viewer-shortcode' ) );
		switch ( $action ) {
	...
	public function add_sidebar( $name, $id = '' ) {
		...
		if ( isset( $registered_sidebars[ $id ] ) ) {
			return new \WP_Error( 'sidebar-exists', __( 'Sidebar with the same ID is already registered.', 'pdfjs-viewer-shortcode' ) );
		}
		...
		if ( ! $success ) {
			return new \WP_Error( 'update-failed', __( 'Failed to update option storing your custom sidebars. Please contact support.', 'pdfjs-viewer-shortcode' ) );
		}
	...
	public function remove_sidebar( $id ) {
		...
		if ( isset( $registered_sidebars[ $id ] ) ) {
			...
			return new \WP_Error( 'sidebar-not-found', __( 'Sidebar not found.', 'pdfjs-viewer-shortcode' ) );
		}
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'carbon-sidebar-manager', 'crbSidebarl10n',
			array(
				'add_sidebar' => __( 'Add Sidebar', 'pdfjs-viewer-shortcode' ),
				'enter_name_of_new_sidebar' => __( 'Please enter the name of the new sidebar:', 'pdfjs-viewer-shortcode' ),
				'remove_sidebar_confirmation' => __( 'Are you sure you wish to remove this sidebar?', 'pdfjs-viewer-shortcode' ),
			)
		);
	}
}
